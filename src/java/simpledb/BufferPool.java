...
public class BufferPool {
    ...
    private int numUsedPages=0;
    private ConcurrentHashMap<PageId,Page> pages=new ConcurrentHashMap<PageId,Page>();
    private ConcurrentLinkedDeque<PageId> queue=new ConcurrentLinkedDeque<PageId>();
    /**
    ...
    public  Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        //System.out.println("Table ID: "+pid.getTableId());
        DbFile dbf=catalog.getDatabaseFile(pid.getTableId());
        if(!pages.containsKey(pid)){
            //System.out.println("not cached yet");
            Page page=dbf.readPage(pid);
            if(numUsedPages<numPages){ 
                updateBuffer(pid,page);    
            }
            else{
    ...
    public void insertTuple(TransactionId tid, int tableId, Tuple t)
        ...
        ArrayList<Page>ps=df.insertTuple(tid, t);
        for(Page p:ps){
    ...
    public  void deleteTuple(TransactionId tid, Tuple t)
        ...
        DbFile df=Database.getCatalog().getDatabaseFile(tableId);
        ArrayList<Page>ps=df.deleteTuple(tid, t);
        for(Page p:ps){
            p.markDirty(true, tid);
            pages.put(p.getId(),p);
        }
    ...
    */
    ...
    //--helper function--
    private void updateBuffer(PageId pid, Page page){
        //pages.remove(pid,page);
        queue.remove(pid);
        pages.put(pid,page);
        queue.add(pid);

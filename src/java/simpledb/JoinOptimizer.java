...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp==Predicate.Op.EQUALS){
            ...
            else card=card1>card2?card1:card2;
        }
        else{
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        //Replace the following
            if(joins.size()==0) return new Vector<LogicalJoinNode>();    
            Set<LogicalJoinNode> joinSet=new HashSet<LogicalJoinNode>(joins);
            PlanCache pc=new PlanCache();
            System.out.println("joins size: "+joins.size()); 
            for(int i=1;i<=joins.size();i++){
                Set<Set<LogicalJoinNode>> allSub=enumerateSubsets(joins, i);
                for(Set<LogicalJoinNode> sub: allSub){
                    double bestCost=Double.MAX_VALUE;
                    int bestCard=Integer.MAX_VALUE;
                    Vector<LogicalJoinNode> bestPlan=null;
                    for(LogicalJoinNode node:sub){
                        CostCard cc= computeCostAndCardOfSubplan(stats, filterSelectivities,node,sub,bestCost,pc);
                        if(cc!=null){
                            bestCost=cc.cost;
                            bestCard=cc.card;
                            bestPlan=cc.plan;
                    ...
                    //System.out.println("best plan of "+i+" is "+bestPlan);
                    pc.addPlan(sub, bestCost, bestCard, bestPlan);
                }
            }
        Vector<LogicalJoinNode> result=pc.getOrder(joinSet);
        //System.out.println("best plan: "+result);       
        if(explain) printJoins(result,pc,stats, filterSelectivities);
        return result;
    }
